...
class GCMLocSave(nn.Module):
    ...
    def forward(self, rgb, lidar, depth, flagt, u, v, pcl, info):
        ...
        Returns:
            (transl0, rot0, transl0, rot0)  coarse Stage-1 pose (×2 for compat.)
        """
        B = rgb.shape[0]
        ...
        h_collapsed = h_lidar.sum(dim=1, keepdim=True) * flagt  # (B, 1, H, W)
        ...
        pcl_flat = pcl.reshape(B, 3, H * W)

        # ===== Save selected point clouds =====
        # info[i] is a list of strings:
        #   KITTI:     ['{seq:02d}', '{frame:06d}']
        #   Argoverse: ['train4/<log_uuid>', '<timestamp>']
        # → saved to: {save_root}/{info[0]}/{save_name}/{info[1]}.npy
        for i in range(B):
            pcl_i   = pcl_flat[i, :, indices[i]].cpu().detach().numpy()  # (3, topk)
            out_dir = os.path.join(self.save_root, str(info[i][0]), self.save_name)
            os.makedirs(out_dir, exist_ok=True)
            np.save(os.path.join(out_dir, f'{info[i][1]}.npy'), pcl_i.T)  # saved: (topk, 3)

        return transl0, rot0, transl0, rot0
