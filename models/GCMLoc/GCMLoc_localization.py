...
class GCMLocLocalization(nn.Module):
    ...
    def forward(self, rgb: torch.Tensor, lidar: torch.Tensor):
        """
        Online localization forward pass (Stage 2 only).

        Args:
            rgb:   Online RGB image              (B, 3, H, W)
            lidar: Pre-saved filtered GCMLoc map  (B, 1, H, W) — from train_save.py

        Returns:
            transl: (B, 3)   — translation prediction
            rot:    (B, 4)   — normalized rotation quaternion
            w_x:    learnable translation loss weight (from pose_reg_s2)
            w_q:    learnable rotation loss weight    (from pose_reg_s2)
        """
        # ===== Feature Extraction =====
        F_I = self.rgb_backbone(rgb)                              # (B, feat_dim, H_i, W_i)
        F_M, feats_m = self.depth_backbone.extract_gcmloc(lidar)  # (B, feat_dim, H/8, W/8)
        #   feats_m = [f1@H/4, f2@H/8, f3@H/16, f4@H/32]

        # ===== Multi-scale Heatmap Decoder =====
        # Stage1HeatmapDecoder: F_M (primary) + feats_m (skip connections)
        # Output at H/4 (96×320 for KITTI) — spatial attention map
        ch = self.heatmap_head_s2(F_M, feats_m)                  # (B, heatmap_dim_s2, H/4, W/4)

        # ===== Flow Matching =====
        flow_embed, _ = self.flow_s2(F_I, feats_m[:3])           # (B, heatmap_dim_s2, ?, ?)

        # Align flow to heatmap resolution (H/4)
        ...
        xf = (flow_embed * ch).sum(dim=(2, 3))                   # (B, heatmap_dim_s2)

        # ===== Pose Regression =====
        transl, rot = self.pose_reg_s2(xf)

        return transl, rot, self.pose_reg_s2.w_x, self.pose_reg_s2.w_q
