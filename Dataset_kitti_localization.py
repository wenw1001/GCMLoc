...
class DatasetVisibilityKittiSingle(Dataset):
    ...
    def __init__(self, dataset_dir, transform=None, augmentation=False, maps_folder='local_maps',
        ...
        # per-sequence canonical remap tables (precomputed once)
        self._remap = {}
        self._canon_calib = UC.get_canonical_calib() if use_canonical else None
        if use_canonical:
            for d in ['00', '03', '05', '06', '07', '08', '09']:
                self._remap[d] = UC.build_remap_maps(*get_calib_kitti(int(d)).tolist())
        self.GTs_R = {}
        self.GTs_T = {}
        ...
        for dir in ['00', '03', '05', '06', '07', '08', '09']:
            self.GTs_R[dir] = []
            self.GTs_T[dir] = []
            print(f'Loading GTs from {os.path.join(dataset_dir, dir, "poses.csv")}')
            df_locations = pd.read_csv(os.path.join(dataset_dir, dir, 'poses.csv'), sep=',', dtype={'timestamp': str})
            for index, row in df_locations.iterrows():
                # print(f"Checking map: {os.path.join(dataset_dir, dir, self.maps_folder, str(row['timestamp'])+'.npy')}")
                if not os.path.exists(os.path.join(dataset_dir, dir, self.maps_folder, str(row['timestamp'])+'.npy')):
                    # print(f"Missing map for {dir} at timestamp {row['timestamp']}")
                    continue
                if not os.path.exists(os.path.join(dataset_dir, dir, 'image_2', str(row['timestamp'])+'.png')):
                    # print(f"Missing image for {dir} at timestamp {row['timestamp']}")
                    continue
        ...
        if split == 'test':
            ...
            print(f'TEST SET: Loaded {len(self.test_RT)} RTs, all_files: {len(self.all_files)}')
    ...
    def __getitem__(self, idx):
        ...
        pc_path = os.path.join(self.root_dir, run, self.maps_folder, timestamp+'.npy')
        ...
        remap = self._remap[run] if self.use_canonical else None
        try:
        ...
        # print(f"[DEBUG] Inside getitem: rotx={rotx}, transl_x={transl_x}, self.max_t={self.max_t}")
        ...
        #io.show()
        calib = self._canon_calib.clone() if self.use_canonical else get_calib_kitti(int(run))
        if h_mirror:
